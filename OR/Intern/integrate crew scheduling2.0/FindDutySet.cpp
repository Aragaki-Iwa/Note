...
void Path::init() {
	csvComposition* duty_compo = route.front()->segment->composition;
	for (int i = 1; i < route.size(); i++) {
		duty_compo = duty_compo->hirarchy > route[i]->segment->composition->hirarchy ?
						duty_compo : route[i]->segment->composition;
	}

	Label* endLabel = route.back()->labelSet.front();
	flyMin = endLabel->accumuFlyMin;
	workMin = endLabel->accumuWorkMin;

	Node* start_node, *end_node;
	start_node = route.front();
	end_node = route.back();
	startDtLoc = start_node->startDtLoc;
	endDtLoc = end_node->endDtLoc;
	startStation = start_node->depStation;
	endStation = end_node->arvStation;
	startDate = start_node->segment->getDate();
	
	assigned = false;
	composition  = duty_compo;
	for (int i = 0; i < compoModes.size(); i++) {
		if (composition->nameDesc == compoModes[i].name) {
			compoMode = &compoModes[i];
			break;
		}
	}
	
	tempCompoMode = *compoMode;
}
void Path::resetCompoNums() {
	tempCompoMode = *compoMode;
}
...
Path* PathFinder::backtrack(Label* endLabel)
{
	...
	path->assign = false;
	path->requireCrewNum = maxCrewNum;*/
